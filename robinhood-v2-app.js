...
contract RobinhoodV2AutoLiquidityMint is ERC20, Ownable, Pausable, ReentrancyGuard {
    ...
    bool public blockMinterBuyAfterOpen;
    ...
    constructor(
        ...
        uint256 launchTime_, bool preLaunchWhitelistEnabled_, uint256 buyTaxBP_, uint256 sellTaxBP_, uint256 maxBuyAmount_, uint256 maxWalletAmount_, bool blockMinterBuyAfterOpen_
        ...
        blockMinterBuyAfterOpen = blockMinterBuyAfterOpen_;
    ...
    function _update(address from, address to, uint256 amount) internal override {
        ...
        bool opened = tradingOpen || (launchTime != 0 && block.timestamp >= launchTime);
        if (buy || sell) {
            if (!opened) {
        ...
        if (buy && opened && blockMinterBuyAfterOpen && !transferWhitelist[to]) require(walletMintCount[to] == 0, "minter buy blocked");
        uint256 tax;
    ...
    function setBlockMinterBuyAfterOpen(bool value) external onlyOwner { blockMinterBuyAfterOpen = value; }
...
function args(form) {
  ...
  return [
    ...
    launchTs(form), bool(form.preLaunchWhitelistEnabled.value), bp(form.buyTax.value), bp(form.sellTax.value), token(form.maxBuyAmount.value), token(form.maxWalletAmount.value), bool(form.blockMinterBuyAfterOpen.value)
  ];
}
const CONSTRUCTOR_TYPES = ["string","string","uint256","address","address","address","uint8","uint256","uint256","uint256","uint256","uint256","uint256","bool","uint256","bool","uint256","uint256","uint256","uint256","bool"];
...
async function refreshAdmin() {
  const c=state.admin; if(!c)return; const [owner,router,factory,weth,taxWallet,lpReceiver,lpMode,buy,sell,maxBuy,maxWallet,blockMinter,reserve,open,launch,price,userToken,lpToken,lpBP,maxCount,maxPer,minted]=await Promise.all([c.owner(),c.ROUTER(),c.FACTORY(),c.WETH(),c.taxWallet(),c.lpReceiver(),c.lpReceiverMode(),c.buyTaxBP(),c.sellTaxBP(),c.maxBuyAmount(),c.maxWalletAmount(),c.blockMinterBuyAfterOpen(),c.dividendReserve(),c.tradingOpen(),c.launchTime(),c.mintPrice(),c.userTokenPerMint(),c.liquidityTokenPerMint(),c.liquidityEthBP(),c.maxMintCount(),c.maxMintPerWallet(),c.mintedCount()]);
  stats("adminStats",[["Owner",owner],["Router",router],["Factory / WETH",`${factory} / ${weth}`],["税费钱包",taxWallet],["LP 接收方式",Number(lpMode)===1?"Mint 用户接收":`指定钱包：${lpReceiver}`],["买/卖税",`${Number(buy)/100}% / ${Number(sell)/100}%`],["限购",`单笔 ${fmt(maxBuy)} / 持仓 ${fmt(maxWallet)}`],["Mint 地址买入限制",blockMinter?"开盘后禁止买入":"允许买入"],["分红储备",`${fmt(reserve)} ETH`],["开盘",open?`已开盘`:(Number(launch)>0?new Date(Number(launch)*1000).toLocaleString():"未开盘")],["Mint 配置",`${fmt(price)} ETH；到账 ${fmt(userToken)}；加池 ${fmt(lpToken)} + ${Number(lpBP)/100}% ETH`],["Mint 进度",`${minted}/${maxCount}；每钱包 ${maxPer}`]]);
}
async function adminAction(name) {
  ...
  const calls={
    ...
    setBlockMinterBuyAfterOpen:()=>c.setBlockMinterBuyAfterOpen(bool($("blockMinterBuyAfterOpen").value)),
