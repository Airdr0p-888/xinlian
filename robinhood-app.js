...
const TARGET_CHAIN_ID = 4663;
const TARGET_CHAIN = NETWORK_DEFAULTS[TARGET_CHAIN_ID];
...
const explorerUrl = (kind, value) => `${TARGET_CHAIN.explorer}${kind}/${value}`;
const txDone = async (tx, label) => {
  log(`${label} 已提交：${tx.hash}（${explorerUrl("tx", tx.hash)}）`);
  await tx.wait();
  log(`${label} 已确认`);
};
...
async function rewardInfo(contract, fallbackContract = contract) {
  ...
  if (rewardAddress === ZERO) {
    ...
    return { address: ZERO, symbol: mode === 0 ? (defaults?.native || "ETH") : "USDT", decimals: 18, native: true };
  }
...
function launchPriceDetails(form) {
  ...
  const defaults = activeNetworkDefaults();
  const currency = Number(form.elements.mintMode.value) === 0 ? (defaults?.native || "ETH") : "USDT";
...
async function connectWallet() {
  ...
  await switchToRobinhoodChain(injected);
  state.provider = new ethers.BrowserProvider(injected);
  await state.provider.send("eth_requestAccounts", []);
  state.signer = await state.provider.getSigner();
  state.account = await state.signer.getAddress();
  const network = await state.provider.getNetwork();
  if (Number(network.chainId) !== TARGET_CHAIN_ID) throw new Error("钱包没有成功切换到 Robinhood Chain 主网。");
  state.network = network;
  $("walletAddress").textContent = state.account;
  $("networkName").textContent = `${TARGET_CHAIN.name} / chainId ${network.chainId}`;
  $("chainExplorerLink").href = TARGET_CHAIN.explorer;
...
async function switchToRobinhoodChain(injected) {
  const current = String(await injected.request({ method: "eth_chainId" })).toLowerCase();
  if (current === TARGET_CHAIN.chainIdHex) return;
  try {
    await injected.request({ method: "wallet_switchEthereumChain", params: [{ chainId: TARGET_CHAIN.chainIdHex }] });
  } catch (error) {
    if (Number(error?.code) !== 4902) throw new Error("请在钱包中切换到 Robinhood Chain 主网后重试。");
    await injected.request({
      method: "wallet_addEthereumChain",
      params: [{
        chainId: TARGET_CHAIN.chainIdHex,
        chainName: TARGET_CHAIN.name,
        nativeCurrency: { name: "Ether", symbol: TARGET_CHAIN.native, decimals: 18 },
        rpcUrls: [TARGET_CHAIN.rpc],
        blockExplorerUrls: [TARGET_CHAIN.explorer]
      }]
    });
  }
}

async function assertRobinhoodNetwork() {
  if (!state.provider) throw new Error("请先连接钱包。");
  const network = await state.provider.getNetwork();
  if (Number(network.chainId) !== TARGET_CHAIN_ID) {
    throw new Error(`当前网络 Chain ID 为 ${network.chainId}，本页面只允许 Robinhood Chain 主网（${TARGET_CHAIN_ID}）。`);
  }
  state.network = network;
}

async function validateV2Router(address) {
  if (!ethers.isAddress(address) || address === ZERO) throw new Error("请填写正确的 Uniswap V2 Router 地址。");
  await assertRobinhoodNetwork();
  const code = await state.provider.getCode(address);
  if (!code || code === "0x") throw new Error("Router 地址在 Robinhood Chain 上没有合约代码。");
  const router = new ethers.Contract(address, ["function factory() view returns (address)", "function WETH() view returns (address)"], state.provider);
  let factory;
  let weth;
  try {
    [factory, weth] = await Promise.all([router.factory(), router.WETH()]);
  } catch {
    throw new Error("该地址不是兼容的 Uniswap V2 Router：无法读取 factory() 或 WETH()。");
  }
  if (!ethers.isAddress(factory) || factory === ZERO || (await state.provider.getCode(factory)) === "0x") throw new Error("Router 返回的 Factory 地址无效或没有合约代码。");
  if (!ethers.isAddress(weth) || weth === ZERO || (await state.provider.getCode(weth)) === "0x") throw new Error("Router 返回的 WETH 地址无效或没有合约代码。");
  log(`Router 校验通过：${address}；Factory ${factory}；WETH ${weth}`);
  return { factory, weth };
}
...
async function deployContract(form) {
  ...
  await ensureWallet();
  await validateV2Router(form.elements.router.value.trim());
  if (Number(form.elements.mintMode.value) === 1 && !TARGET_CHAIN.usdt) throw new Error("Robinhood Chain 的 USDT 地址尚未配置，本页面当前只允许 ETH Mint。");
  ...
  state.mint = contract;
  log(`部署完成：${address}（${explorerUrl("address", address)}）`);
  await refreshAdmin();
  await refreshMint();
...
async function contractAt(address) {
  await ensureWallet();
  if (!ethers.isAddress(address) || address === ZERO) throw new Error("请输入正确的合约地址。");
  if ((await state.provider.getCode(address)) === "0x") throw new Error("该地址在 Robinhood Chain 上没有合约代码。");
...
async function refreshAdmin() {
  ...
  renderStats("adminStats", [
    ["Owner", owner], ["Pair", pair], ["Mint 模式", Number(mintMode) === 0 ? "ETH" : "USDT"],
    ...
    ["金额限购", buyAmountLimitEnabled ? "开启" : "关闭"], ["单钱包金额上限", `${ethers.formatUnits(maxBuyBaseAmountPerWallet, 18)} ${Number(mintMode) === 0 ? "ETH" : "USDT"}`],
...
const ERROR_TRANSLATIONS = [
  [/not\s*bnb\s*mode/i, "当前合约是 USDT 模式，不支持 ETH Mint"],
  [/not\s*usdt\s*mode/i, "当前合约是 ETH 模式，不支持 USDT Mint"],
  [/bad\s*bnb\s*amount/i, "发送的 ETH 金额不正确，请检查 Mint 价格"],
  ...
  [/no\s*available\s*BNB/i, "无可提取的 ETH"],
